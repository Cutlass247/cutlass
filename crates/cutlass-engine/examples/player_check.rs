...
fn main() -> anyhow::Result<()> {
    ...
    let clips = vec![
        AudioClip { path: path.clone(), start: 0.0, len: 1.0, src_in: 0.0, volume: 1.0, speed: 1.0, audio_offset: 0.0 },
        // 0.5 s gap → silence
        AudioClip { path: path.clone(), start: 1.5, len: 1.0, src_in: 5.0, volume: 1.0, speed: 1.0, audio_offset: 0.0 },
    ];
